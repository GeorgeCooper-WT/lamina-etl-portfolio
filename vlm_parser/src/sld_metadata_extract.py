"""
Extracts top-level site metadata from SLD images using Anthropic Claude and a custom prompt.
Usage:
    python src/sld_metadata_extract.py
...
from schema.SldMetadata import Metadata
...
def load_prompt(prompt_path: Optional[Path] = None) -> str:
    default_path = Path(__file__).parent.parent / "prompts" / "sld_metadata_prompt"
...
def parse_site_metadata(
    image_path: str, prompt_path: Optional[Path] = None
) -> Metadata:
    prompt = load_prompt(prompt_path)
    try:
        response = _call_llm(image_path, prompt, Metadata)
        return Metadata.model_validate(response)
...
if __name__ == "__main__":
    ...
    if not image_files:
        ...
        result = parse_site_metadata(str(image_path))
