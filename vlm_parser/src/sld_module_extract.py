"""
Extracts module metadata from SLD images using Anthropic Claude and a custom prompt.
Usage:
    python src/sld_module_extract.py
...
from schema.SldModule import SolarModule
from schema.SldModule import ModuleExtractResponse
...
def load_prompt(prompt_path: Optional[Path] = None) -> str:
    default_path = Path(__file__).parent.parent / "prompts" / "sld_module_prompt"
...
) -> ModuleExtractResponse:
    prompt = load_prompt(prompt_path)
    try:
        response = _call_llm(image_path, prompt, ModuleExtractResponse)
        return ModuleExtractResponse.model_validate(response)
...
if __name__ == "__main__":
    ...
    if not image_files:
        ...
        result = parse_module_metadata(str(image_path))
