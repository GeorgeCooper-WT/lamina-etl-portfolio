"""
Extracts string metadata from SLD images using Anthropic Claude and a custom prompt.
Usage:
    python src/sld_string_extract.py
...
from schema.SldString import String
from schema.SldString import StringExtractResponse
...
def load_prompt(prompt_path: Optional[Path] = None) -> str:
    default_path = Path(__file__).parent.parent / "prompts" / "sld_string_prompt"
...
def parse_string_metadata(
    image_path: str, prompt_path: Optional[Path] = None
) -> StringExtractResponse:
    prompt = load_prompt(prompt_path)
    try:
        response = _call_llm(image_path, prompt, StringExtractResponse)
        return StringExtractResponse.model_validate(response)
...
if __name__ == "__main__":
    ...
    if not image_files:
        ...
        result = parse_string_metadata(str(image_path))
