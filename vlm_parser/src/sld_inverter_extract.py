"""
Extracts inverter metadata from SLD images using Anthropic Claude and a custom prompt.
Usage:
    python src/sld_inverter_extract.py
...
from schema.SldInverter import InverterExtractResponse
...
def load_prompt(prompt_path: Optional[Path] = None) -> str:
    default_path = Path(__file__).parent.parent / "prompts" / "sld_inverter_prompt"
...
def parse_inverter_metadata(
    image_path: str, prompt_path: Optional[Path] = None
) -> InverterExtractResponse:
    prompt = load_prompt(prompt_path)
    try:
        response = _call_llm(image_path, prompt, InverterExtractResponse)
        return InverterExtractResponse.model_validate(response)
...
if __name__ == "__main__":
    ...
    if not image_files:
        ...
        result = parse_inverter_metadata(str(image_path))
